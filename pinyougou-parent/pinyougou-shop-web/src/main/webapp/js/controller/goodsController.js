...
app.controller('goodsController', function ($scope, $controller, $location, goodsService,
    ...
    $scope.selectGoodsList=function () {
        goodsService.selectGoodsList().success(function (response) {
            $scope.goodsList = response;
        })
    }
    //根据goodsId改变更改name
    $scope.$watch('$scope.entity.goodsId',function (newValue, oldValue) {
        $scope.goodsName = $scope.goodsList[newValue];
    })
});
